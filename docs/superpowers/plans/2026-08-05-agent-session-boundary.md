...
> ⚠️ **ВЫПОЛНЕНО 2026-08-05; реализация РАЗОШЛАСЬ с этим документом. Актуальное описание — абзац «Граница переписки и время сообщений» в `CLAUDE.md`, а не этот план.**
> По ходу работы в самом плане нашлись три настоящих дефекта, плюс ревью добавило четыре правки. Отличия от текста ниже:
> 1. `formatGap` округляет часы до БЛИЖАЙШЕГО, а не вниз: разрыв меряется до начала серии, поэтому «неделя молчания» — это 6 д 23:59:40, и `Math.floor` давал «6 дней».
> 2. `formatStamp` отвергает `n <= 0` (плановое `Number.isFinite(ms)` пропускало `null`: `Number(null) === 0` → рендерилась эпоха `[01.01 03:00]`) и `n > 4e10` (иначе `Intl` БРОСАЕТ на выходе за диапазон `Date` и роняет ход, а ts в миллисекундах даёт правдоподобную ложную метку).
> 3. `stripStamp` срезает метку в начале КАЖДОЙ строки, а не только первой, и с учётом невидимых символов: реплики серии склеиваются через `\n`, поэтому подделка «второй реплики» проходила одним переводом строки, а ведущий `​` — мимо `\s`.
> 4. Порог — `>= 6 ч` (в спеке «> 6 ч»); есть тест «порог включительно».
> 5. Блок «НАЧАЛО НОВОЙ ПЕРЕПИСКИ» рендерится ТОЛЬКО при измеренном разрыве (`gapText` непуст) и НЕ велит «поздороваться как в начале диалога» и перепроверять цены инструментами — обе формулировки ломали поведение (отыгрыш шаблона первого сообщения; `list_services` в боевом `catalogMode` не существует).
> 6. `loadTranscript` берёт `COALESCE(msg_ts, EXTRACT(EPOCH FROM (created_at AT TIME ZONE 'Europe/Moscow'))::bigint)` в SELECT и ORDER BY: `msg_ts` nullable, а `ORDER BY … DESC` в PG — `NULLS FIRST`, из-за чего NULL-строка вставала в хвост транскрипта и молча выключала границу переписки навсегда.
> 7. Сверх плана: `worker.js` срезает `OPERATOR_MARK` перед care-промптом; `hasPriorAssistant` глушится ровно при отрендеренном блоке; финальные реплики прогоняются через `stripStamp`.
