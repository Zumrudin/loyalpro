...
async function run(salonId, input, ctx = {}) {
  ...
  if (catalog && Array.isArray(catalog.services) && catalog.services.length) {
    ...
    // Обобщённая услуга по умолчанию (правило «ПРЕПАРАТ/ФИЛЛЕР/ЗОНУ НЕ УТОЧНЯЕМ»,
    ...
    if (!input.patient_named_service) {
      ...
      if (g) {
        return {
          ...
          error: `Пациент не называл препарат (${g.brands.join(' ')}). По правилу «ПРЕПАРАТ/ФИЛЛЕР/ЗОНУ НЕ УТОЧНЯЕМ» ` +
