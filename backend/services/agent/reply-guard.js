...
// «не помещается/не влезает» — отказ для ЦЕПОЧКИ услуг (инцидент 2026-09-22,
// 79265824264: «на 18:00 все три процедуры подряд уже не помещаются» при 18:00
// в starts стыковки) — прежний список знал только формы про одиночный слот.
const UNAVAILABLE_RE =
  /(занят[а-яё]*|недоступн[а-яё]*|нет\s+свободн[а-яё]*|уже\s+нет|не\s+получ[а-яё]*|не\s+выйдет|не\s+остал[а-яё]*|нет\s+(?:окошк|окн|врем)[а-яё]*|не\s+помещ[а-яё]*|не\s+помест[а-яё]*|не\s+влез[а-яё]*)/iu;
...
//
// СПИСОК НЕ ЛИНГВИСТИЧЕСКИ ПОЛНЫЙ: закрытый набор форм собран под реально
// встреченные и протестированные фразы, а не выведен грамматически — например,
// будущее время 1/2 лица мн. числа («перенесём», «перенесёте») в списке нет, и
// «Перенесёте мою запись на пятницу?» гейт НЕ поймает. Это фолс-негатив
// (безопасное направление — гейт молчит, как и до этой правки), а не фолс-
// позитив, поэтому не блокирует: пробелы в покрытии обнаруживаются тем же
// логом, что и остаточный риск составного сообщения выше, — при необходимости
// список можно расширить, не меняя структуру (закрытая альтернация, не суффикс).
