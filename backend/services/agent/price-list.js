...
// Страховочная фраза, когда НИ ОДНО фото хода не ушло (Chatpush ответил
// ошибкой). ЗАЧЕМ детерминированно, а не правилом промпта: к моменту сбоя
// реплика модели уже доставлена и в ней сказано «отправила вам цены» — пациент
// остаётся с обещанием и пустым чатом. Ровно так прошёл инцидент 2026-09-07
// (79165944414, MAX): 422 на оба файла, клиентка через полчаса ответила
// «Не удалось». Фраза НЕ называет причину («канал», «ошибка API») — пациенту
// это ничего не даёт, — и не обещает прислать фото позже: повтора нет.
// Ссылка подставляется, только если салон её завёл (agent_settings.price_list_url).
function photoFailureText(url) {
  const link = String(url || '').trim();
  const tail = link ? ` Полный прайс есть на сайте: ${link}.` : '';
  return `Прошу прощения, фото прайса не отправилось.${tail} Напишите, пожалуйста, какая процедура вас интересует, — подскажу стоимость текстом.`;
}
