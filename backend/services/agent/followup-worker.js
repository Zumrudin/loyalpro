...
// ЯКОРЯ, поэтому после долгого простоя процесса (pm2 restart, откладывание
// рычагом) момент финала может УЖЕ пройти к тому времени, как ушло
// напоминание — и следующий же тик отправил бы второе сообщение через минуту
// после первого. Финал сдвигается вперёд, а не отменяется: он короткий и
// закрывающий, но подряд с напоминанием он выглядит спамом.
...
async function buildNudgeText(d, row, messages) {
  ...
  if (decision.action !== 'send') {
    ...
    if (decision.action === 'escalate') {
      d.log.warn(`followup #${row.id}: модель вернула escalate вне контракта промпта — просто молчим`);
    }
...
async function processOne(row, deps = defaultDeps) {
  ...
  try {
    ...
    if (isFinal) {
      ...
      const finalAt = !due || due < floor ? floor : due;
      marked = await d.db.query(
        `UPDATE agent_followups
