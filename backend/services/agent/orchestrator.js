...
async function runDialogInner(salonId, dialogKey, opts = {}, bag = {}) {
  ...
  for (let attempt = 0; attempt <= MAX_REGEN; attempt++) {
    ...
    // «или утро, или вечер» звучала парой сообщений раньше (инцидент 2026-09-19,
    // ход 6). Реплики Милы НЕ подмешиваются: её собственный текст «на вечер тоже
    // всё расписано» тоже содержит слово «вечер» и подтверждал бы сам себя.
    // Ограничение по числу — в patient-time.parseDayPartFromRecent (DAY_PART_LOOKBACK).
