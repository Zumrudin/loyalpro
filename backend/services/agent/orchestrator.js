...
const BOOKED_STATE_CLAIM = /(вы\s+записаны|записал[аи]?\s+вас|оформлен[оа]|подтверждена|запись\s+(создан|оформлен|подтвержден))/i;
...
async function runDialogInner(salonId, dialogKey, opts = {}, bag = {}) {
  ...
  for (let attempt = 0; attempt <= MAX_REGEN; attempt++) {
    ...
    const existsHonest = readBookings || bookingsExist === true;
    const cancelledHonest = bookingsExist === false;
    const falseSuccess = !escalated && !writeSucceeded
      && (COMPLETION_CLAIM.test(allReplies)
        || (!existsHonest && BOOKED_STATE_CLAIM.test(allReplies))
        || (!cancelledHonest && CANCELLED_STATE_CLAIM.test(allReplies)));
