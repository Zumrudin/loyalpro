...
describe('КАКОЕ ВРЕМЯ ПРЕДЛАГАТЬ ПЕРВЫМ (плотная запись)', () => {
  ...
  test('правила выбора и альтернативы берут время из offer_slots', () => {
    expect(p).toContain('и время ДОСЛОВНО из его offer_slots (если он пуст — из slots), и спроси, к кому удобнее записать');
    expect(p).toContain('и время ДОСЛОВНО из его offer_slots (если он пуст — из slots), ясно обозначив');
  });
