...
describe('экономия вызовов инструментов', () => {
  ...
  test('catalog-режим: «инд.» цифрой не озвучивается и исключена из подсчёта диапазона направления', () => {
    ...
    // Диапазоны направлений с 10.08.2026 считает КОД (блок «ДИАПАЗОНЫ ЦЕН» в каталоге),
    // и «инд.» исключена там же; в промпте исключение осталось на запасном пути —
    // когда направления в готовом блоке нет и модель считает по category_path сама.
    expect(p).toMatch(/не включая услуги с ценой «инд\.»/i);
  });
