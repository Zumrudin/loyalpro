...
describe('buildSystemPrompt', () => {
  ...
  test('привязывает предлагаемое время к массиву slots (не выдумывать/округлять)', () => {
    const p = buildSystemPrompt({});
    expect(p).toMatch(/ДОСЛОВНО скопированных из ответа get_available_slots/i);
  ...
  // Инцидент 2026-08-05: «за весь диалог» заменено на «в рамках ТЕКУЩЕГО
  // разговора» — окно транскрипта без границы по времени делало недельной
  // давности переписку «продолжающейся». Запрет на ПОВТОРНОЕ приветствие
  // внутри одного разговора не ослаблен, добавлено только исключение.
  test('здоровается только один раз за разговор — без повторного «Здравствуйте, {имя}»', () => {
    const p = buildSystemPrompt({});
    expect(p).toMatch(/Здоровайся ТОЛЬКО ОДИН раз в рамках ТЕКУЩЕГО разговора/i);
    expect(p).toMatch(/ДАЖЕ когда только что узнала имя/i);
    expect(p).toMatch(/началась НОВАЯ ПЕРЕПИСКА[^]{0,40}поздоровайся заново/i);
  });
...
describe('экономия вызовов инструментов', () => {
  ...
  test('catalog-режим: «инд.» цифрой не озвучивается и исключена из подсчёта диапазона направления', () => {
    ...
    // Диапазоны направлений с 10.08.2026 считает КОД (блок «ДИАПАЗОНЫ ЦЕН» в
    // каталоге), исключение «инд.» — обещание его заголовка; правило промпта
    // запрещает пересчёт, а отсутствие направления в блоке (= у всех услуг
    // «инд.»/нет цены) ведёт в явный терминал «стоимость определит врач»,
    // а не в самостоятельный подсчёт по category_path (тупик → выдумка).
    const { RANGES_HEADER } = require('./services/agent/catalog-block');
    expect(RANGES_HEADER).toMatch(/услуги «инд\.» и цена единицы препарата в диапазоны не входят/i);
    expect(p).toMatch(/НИКОГДА не пересчитывай диапазон сама/i);
    expect(p).toMatch(/диапазон НЕ выдумывай и по каталогу НЕ пересчитывай/i);
    expect(p).toMatch(/стоимость определит врач на консультации, и предложи записаться/i);
  });
...
describe('выбор специалиста делает пациент', () => {
  ...
  // Прямая коллизия правил (диагностика 2026-08-10): правило 11 запрещает
  // перечислять специалистов «по своей инициативе», а правило «ВЫБОР СПЕЦИАЛИСТА
  // ДЕЛАЕТ ПАЦИЕНТ» требует перечислить ВСЕХ из staff_options. Модель разруливала
  // противоречие сама — оговорка обязана стоять в самом правиле 11.
  test('правило 11 содержит оговорку про staff_options — коллизии с выбором специалиста нет', () => {
    expect(p).toMatch(/НЕ перечисляй специалистов по своей инициативе[\s\S]{0,600}staff_options/);
  });
...
// ответила по делу БЕЗ приветствия. Транскрипт грузится LIMIT 20 и БЕЗ окна по
// времени, поэтому переписка недельной давности выглядела продолжающейся, а
// правило «здоровайся один раз за ВЕСЬ ДИАЛОГ» сработало на ложной посылке.
describe('граница новой переписки', () => {
  ...
  // Без ИЗМЕРЕННОГО разрыва блока быть не должно. gapText===null — это ветка
  // «в окне только сообщения клиента»: либо первое в жизни обращение (там
  // всё закрыто правилом приветствия и разделом ИДЕНТИФИКАЦИЯ, а голое «просто
  // поздоровайся и переходи к делу» строкой выше образца первого сообщения
  // стоило бы представления и вопроса об имени), либо WhatsApp-диалог, где наши
  // исходящие не персистятся (dispatcher кладёт реплику только в pendingReplies)
  // и через 30 минут TTL транскрипт честно выглядит «только входящие» — блок
  // предписал бы поздороваться посреди живого разговора.
  test('newSession без измеренного разрыва → блока НЕТ', () => {
  ...
  // Блок стоит ОДНОЙ строкой выше образца первого сообщения («Я Мила,
  // виртуальный администратор…»), поэтому «как в начале диалога» читалось бы
  // как разрешение отыграть весь шаблон — вместе с повторным запросом имени.
  test('блок не велит отыгрывать шаблон первого сообщения', () => {
    const p = buildSystemPrompt({ ...base, session: { newSession: true, gapText: '8 дней' } });
    expect(p).not.toContain('как в начале диалога');
    expect(p).toContain('Представляться второй раз');
  });

  // Перепроверять «цены инструментами» нельзя: в боевом catalogMode
  // list_services не существует, а вызовы инструментов лимитированы.
  test('блок не требует перепроверять цены инструментами', () => {
    const p = buildSystemPrompt({ ...base, session: { newSession: true, gapText: '8 дней' } });
    const block = p.slice(p.indexOf('НАЧАЛО НОВОЙ ПЕРЕПИСКИ'));
    expect(block).not.toMatch(/цены[^.]*перепроверяй/);
  });

  // Правило приветствия ссылается на блок словами, а не по ссылке: переименуют
  // заголовок — правило будет указывать в пустоту. Связку закрепляем тестом,
  // как сделано с OPERATOR_MARK и formatStamp.
  test('исключение в правиле приветствия ссылается на реальный заголовок блока', () => {
    const p = buildSystemPrompt({ ...base, session: { newSession: true, gapText: '7 дней' } });
    const heading = 'НОВОЙ ПЕРЕПИСКИ';
    expect(p).toContain(`НАЧАЛО ${heading}`);          // заголовок блока
    expect(p).toContain('НОВАЯ ПЕРЕПИСКА');            // ссылка из правила приветствия
  });
...
// («Доброе утро! Можете записать меня на эпиляцию…»), а Мила ответила «Да, на
// 12 августа в 16:00 есть свободное время…» — ни приветствия, ни представления.
// Приветствие было чисто промптовым правилом: детерминированного признака
// «это первое сообщение» в промпте не было вовсе (блок НАЧАЛО НОВОЙ ПЕРЕПИСКИ
// рендерится только при ИЗМЕРЕННОМ разрыве, а на первом обращении мерить нечего).
describe('первое обращение', () => {
  const base = { salonName: 'PERI CLINIC', today: '6 августа 2026, среда', now: '08:27' };

  test('блок появляется при firstContact', () => {
    const p = buildSystemPrompt({ ...base, firstContact: true });
    expect(p).toContain('ПЕРВОЕ ОБРАЩЕНИЕ');
  });

  test('без firstContact блока нет', () => {
    expect(buildSystemPrompt({ ...base, firstContact: false })).not.toContain('ПЕРВОЕ ОБРАЩЕНИЕ');
    expect(buildSystemPrompt(base)).not.toContain('ПЕРВОЕ ОБРАЩЕНИЕ');
  });

  // Блок волатильный (меняется от хода к ходу) — выше «Сейчас по Москве» его
  // класть нельзя: порвёт кэшируемый префикс промпта.
  test('блок стоит ПОСЛЕ «Сейчас по Москве» — префикс-кэш цел', () => {
    const p = buildSystemPrompt({ ...base, firstContact: true });
    expect(p.indexOf('ПЕРВОЕ ОБРАЩЕНИЕ')).toBeGreaterThan(p.indexOf('Сейчас по Москве'));
  });

  // Ровно то, из-за чего блок не сделали в прошлый раз (комментарий в
  // system-prompt.js): голое «поздоровайся и переходи к делу» стоило бы
  // представления и вопроса об имени КАЖДОМУ новому пациенту. Поэтому блок
  // обязан отсылать к разделу ИДЕНТИФИКАЦИЯ ПАЦИЕНТА, а не заменять его.
  test('блок отсылает к разделу идентификации, а не диктует свой текст', () => {
    const p = buildSystemPrompt({ ...base, firstContact: true });
    const block = p.slice(p.indexOf('ПЕРВОЕ ОБРАЩЕНИЕ'));
    expect(block).toContain('ИДЕНТИФИКАЦИЯ ПАЦИЕНТА');
    expect(p).toContain('ИДЕНТИФИКАЦИЯ ПАЦИЕНТА:');   // раздел реально существует
  });

  // Модель прочитала «приветствие уже прозвучало» как «пациент поздоровался».
  test('блок явно говорит, что приветствие пациента не считается', () => {
    const block = buildSystemPrompt({ ...base, firstContact: true });
    expect(block.slice(block.indexOf('ПЕРВОЕ ОБРАЩЕНИЕ'))).toMatch(/приветстви[ея][^.]*пациента[^.]*не считается/i);
  });

  // Правило «здоровайся один раз» не должно засчитывать приветствие ПАЦИЕНТА:
  // именно этой двусмысленностью объясняется молчание Милы в ответ на «Доброе утро».
  test('правило приветствия считает только СВОИ приветствия', () => {
    const p = buildSystemPrompt(base);
    expect(p).not.toContain('Если приветствие уже прозвучало');
    expect(p).toMatch(/ТЫ САМА уже поздоровалась/);
  });

  // Ветка известного пациента (карточка есть) была самой слабой: только
  // «Образец ПЕРВОГО сообщения», без единого императива — в отличие от двух
  // веток незнакомого пациента, где прямо сказано «В первом ответе, помимо
  // приветствия, …». Инцидент случился ровно на ней (client #2458, Юлия).
  test('ветка известного пациента требует поздороваться в первом ответе', () => {
    const p = buildSystemPrompt({ ...base, clientName: 'Юлия', phoneKnown: true });
    const block = p.slice(p.indexOf('ИДЕНТИФИКАЦИЯ ПАЦИЕНТА:'));
    expect(block).toMatch(/перв[а-яё]+ (сообщени|ответ)[а-яё]*[^.]*(поздоровайся|приветств)/i);
  });
});
...
describe('пояснение метки времени', () => {
  ...
  // с метками времени немонотонность стала ВИДНА модели.
  test('промпт предупреждает о немонотонности времён в истории', () => {
    const p = buildSystemPrompt({ salonName: 'PERI CLINIC', today: 'x', now: '10:00' });
    expect(p).toContain('не всегда идут строго по возрастанию времени');
  });
});

// Инцидент 2026-08-06 (79037504378): «Наш адрес: 2-й Троицкий переулок, 6Ас4»
// после успешной записи — адрес выдуман, search_knowledge_base за ход не
// вызывался. Промпт-правило и детерминированный гейт связаны тестом: правило
// без гейта модель уже нарушала, гейт без правила резал бы её ответы молча.
describe('адрес и контакты клиники', () => {
  const addressGuard = require('./services/agent/address-guard');
  const p = () => buildSystemPrompt({ salonName: 'PERI CLINIC', today: 'x', now: '10:00' });

  test('правило запрещает и выдумывать адрес, и называть его по своей инициативе', () => {
    const block = p().slice(p().indexOf('АДРЕС, ТЕЛЕФОН'));
    expect(block).toContain('search_knowledge_base');
    expect(block).toMatch(/НЕ называй адрес и контакты по своей инициативе/);
    expect(block).toMatch(/После оформления записи адрес НЕ дописывай/);
  });

  test('гейт узнаёт ровно те формы, которые перечисляет правило', () => {
    for (const s of ['Наш адрес: ...', 'Ближайшее метро — Домодедовская.',
      'Как до нас добраться: ...', 'Мы находимся в центре.']) {
      expect(addressGuard.MARKER_RE.test(s)).toBe(true);
    }
  });
});
...
describe('КАКОЕ ВРЕМЯ ПРЕДЛАГАТЬ ПЕРВЫМ (плотная запись)', () => {
  ...
  // Правки в правилах ВЫБОР СПЕЦИАЛИСТА и АЛЬТЕРНАТИВНЫЙ СПЕЦИАЛИСТ ничем больше
  // не закреплены: откат ровно этих двух мест обратно на «времени из его slots»
  // оставил бы весь сьют зелёным — а именно там у модели худшая история
  // инцидентов (01.08, 04.08, 06.08).
  test('правила выбора и альтернативы берут время из offer_slots', () => {
    expect(p).toContain('и время ДОСЛОВНО из его offer_slots (если у него стоит free_day:true');
    expect(p).toContain('и спроси, к кому удобнее записать');
    expect(p).toContain('назови имя специалиста из alternative_staff и время ДОСЛОВНО из его offer_slots');
  });

  // Правка 07.08: у блока полезны РОВНО два времени (перед началом и после конца),
  // поэтому offer_slots часто содержит одно — и модель не должна «дополнять» его
  // соседним окном из slots. Ровно на это жаловался салон: «13:00 и 13:30».
  test('одно время в offer_slots — норма, соседние из slots не дописываются', () => {
    expect(p).toContain('НЕ добавляй к нему соседние времена из slots');
    expect(p).toContain('13:00 или 13:30');
  });

  // Свободный день: «плотного» времени не существует, выбор половины дня — за
  // пациентом (решение салона 07.08). Правило связано с полями инструмента.
  test('правило про свободный день называет free_day, day_part и половины дня', () => {
    expect(p).toContain('ДЕНЬ СВОБОДЕН ЦЕЛИКОМ');
    expect(p).toContain('free_day:true');
    expect(p).toContain('day_part');
    for (const part of Object.keys(require('./services/agent/slot-density').DAY_PARTS)) {
      expect(p).toContain(part);
    }
    expect(p).toContain('day_part_empty:true');
  });
...
describe('ПРАЙС-ЛИСТЫ В КАРТИНКАХ', () => {
  ...
  test('запрет пересказывать картинку и слать прайс по своей инициативе', () => {
    const p = buildSystemPrompt({ priceListBlock: BLOCK });
    // Проверки привязаны к ФОРМУЛИРОВКАМ правила прайса, а не к общим словам:
    // и «не пересказывай», и «по своей инициативе» встречаются в базовом промпте
    // сами по себе, и на них тест проходил бы даже без блока прайсов вовсе.
    expect(p).toMatch(/НЕ пересказывай содержимое присланного листа/);
    expect(p).toMatch(/НЕ отправляй прайс по своей инициативе/);
    const bare = buildSystemPrompt({});
    expect(bare).not.toMatch(/НЕ пересказывай содержимое присланного листа/);
    expect(bare).not.toMatch(/НЕ отправляй прайс по своей инициативе/);
  });
  ...
  // Находка B ревью: блок прайсов обязан стоять в СТАБИЛЬНОЙ (кэшируемой) части
  // промпта — сразу за каталогом услуг и ДО ЛЮБОЙ волатильной части, иначе
  // рвётся префикс-кэш провайдера на каждом ходу. Проверено только глазами до
  // этого теста — если блок переедет в хвост (рядом с «АКТИВНЫЕ ВАРИАНТЫ
  // СТЫКОВКИ» / «ЖУРНАЛ ТВОИХ ДЕЙСТВИЙ» / «АКТУАЛЬНЫЕ ЗАПИСИ ПАЦИЕНТА»), тест упадёт.
  test('блок стоит РАНЬШЕ волатильных частей — рядом с каталогом, до «Сейчас по Москве» и хвостовых блоков', () => {
    const catalogBlock = 'КАТАЛОГ УСЛУГ КЛИНИКИ (тест)\n1|Чистка|60|6500|Уход|7';
    const p = buildSystemPrompt({
      catalogBlock,
      ...
      now: '10:00',
      clientName: 'Зумрудин',
      activeOffers: ['o1 — 30.07: 10:30 «Чистка» (Юлия)'],
      toolMemory: ['get_available_slots(...) 500ms ok'],
      liveBookings: [],
    });
    const at = p.indexOf('ПРАЙС-ЛИСТЫ В КАРТИНКАХ');
    expect(at).toBeGreaterThan(-1);
    // Сразу за каталогом услуг (стабильный префикс).
    expect(p.indexOf('КАТАЛОГ УСЛУГ КЛИНИКИ')).toBeLessThan(at);
    // Раньше строки «Сейчас по Москве» (меняется ежеминутно — рвёт кэш чаще всего).
    expect(at).toBeLessThan(p.indexOf('Сейчас по Москве'));
    // Раньше идентификации пациента (тоже волатильная часть).
    expect(at).toBeLessThan(p.indexOf('ИДЕНТИФИКАЦИЯ ПАЦИЕНТА'));
    // Раньше всех хвостовых блоков.
    expect(at).toBeLessThan(p.indexOf('АКТИВНЫЕ ВАРИАНТЫ СТЫКОВКИ'));
    expect(at).toBeLessThan(p.indexOf('ЖУРНАЛ ТВОИХ ДЕЙСТВИЙ'));
    expect(at).toBeLessThan(p.indexOf('АКТУАЛЬНЫЕ ЗАПИСИ ПАЦИЕНТА'));
  });
});

// Инцидент 2026-08-10 (79776646672): пациентка ответила «5» на опрос об оценке
// визита, а Мила поздоровалась и спросила «чем могу помочь?». Опрос был первым
// в окне транскрипта и срезался вместе с остальными ведущими исходящими
// (Messages API требует user первым) — модель увидела ровно одну строку «5».
describe('блок «ПРЕДЫДУЩИЕ СООБЩЕНИЯ КЛИНИКИ»', () => {
  const SURVEY = 'Просим Вас оценить обслуживание, отправив в ответ сообщение с цифрой от 2 до 5';

  // Заголовок сверяем ЦЕЛИКОМ: правило «ОЦЕНКА ВИЗИТА» ссылается на блок по
  // короткому имени, и проверка по нему зеленела бы даже без самого блока.
  const HEAD = 'ПРЕДЫДУЩИЕ СООБЩЕНИЯ КЛИНИКИ ЭТОМУ ПАЦИЕНТУ:';

  test('срезанные исходящие попадают в промпт', () => {
    const p = buildSystemPrompt({ today: '2026-08-10', now: '09:00', leadingClinic: [SURVEY] });
    expect(p).toContain(HEAD);
    expect(p).toContain(SURVEY);
  });

  test('срезать нечего → блока нет вовсе', () => {
    const p = buildSystemPrompt({ today: '2026-08-10', now: '09:00', leadingClinic: [] });
    expect(p).not.toContain(HEAD);
  });

  // Кэш префикса у провайдера: промпт без блока обязан остаться ПРЕФИКСОМ
  // промпта с блоком — то же требование, что у всех хвостовых блоков.
  test('блок стоит в САМОМ хвосте, ниже актуальных записей', () => {
    const base = { today: '2026-08-10', now: '09:00', liveBookings: [], toolMemory: ['x'] };
    const without = buildSystemPrompt(base);
    const withBlock = buildSystemPrompt({ ...base, leadingClinic: [SURVEY] });
    expect(withBlock.startsWith(without)).toBe(true);
    expect(withBlock.indexOf('АКТУАЛЬНЫЕ ЗАПИСИ ПАЦИЕНТА:'))
      .toBeLessThan(withBlock.indexOf(HEAD));
  });
});

describe('правило «ОЦЕНКА ВИЗИТА»', () => {
  const p = () => buildSystemPrompt({ today: '2026-08-10', now: '09:00' });

  test('правило есть и разводит высокую и низкую оценку', () => {
    expect(p()).toContain('ОЦЕНКА ВИЗИТА');
    expect(p()).toMatch(/поблагодар/i);
    expect(p()).toContain('escalate_to_operator');
  });

  // Текст опроса задаёт салон в YClients, и в промпте он не дублируется —
  // сам опрос приезжает блоком «ПРЕДЫДУЩИЕ СООБЩЕНИЯ КЛИНИКИ». Но диапазон
  // цифр правило обязано называть: без него «5» неотличима от номера дня.
  test('правило называет диапазон оценок', () => {
    expect(p()).toMatch(/от\s*2\s*до\s*5/);
  });
});
...
// ── Связь правила «Цена НАПРАВЛЕНИЯ» с РЕАЛЬНЫМ заголовком блока диапазонов ──
// Правило ссылается на блок по имени «ДИАПАЗОНЫ ЦЕН»: без этой сверки
// переименование RANGES_HEADER в catalog-block.js оставит промпт со ссылкой на
// несуществующий блок при всех зелёных тестах (та же готча, что у «ОЦЕНКИ ВИЗИТА»).
describe('правило цены направления ↔ реальный блок каталога', () => {
  test('ссылка из правила есть в заголовке настоящего renderCatalogBlock', () => {
    const { renderCatalogBlock, RANGES_HEADER } = require('./services/agent/catalog-block');
    const realBlock = renderCatalogBlock([
      { yc_id: 1, title: 'Чистка лица', duration_min: 60, price_min: 5000, price_max: 5000,
        category_path: ['Уходы'], staff: [{ yc_id: 5, name: 'Юлия', price_min: 5000, price_max: 5000 }] },
    ]);
    const p = buildSystemPrompt({ catalogBlock: realBlock });
    // Промпт содержит настоящий заголовок целиком…
    expect(p).toContain(RANGES_HEADER);
    // …и подстрока-ссылка из правила — часть этого заголовка.
    const ref = 'блоке «ДИАПАЗОНЫ ЦЕН»';
    expect(p).toContain(ref);
    expect(RANGES_HEADER).toContain(ref.replace(/^блоке «/, '').replace(/»$/, ''));
  });

  test('catalog-режим: правило БОТУЛИНОТЕРАПИИ отсылает к блоку, мужской прайс — только при его наличии', () => {
    const p = buildSystemPrompt({ catalogBlock: PRICE_CATALOG });
    // Правило 5 ревью: в catalog-режиме диапазон ботулинотерапии — из готового
    // блока, без самостоятельного пересчёта (иначе противоречие с «Цена НАПРАВЛЕНИЯ»).
    expect(p).toMatch(/назови диапазон направления из блока «ДИАПАЗОНЫ ЦЕН» — сама не пересчитывай/i);
    // Правило 14 ревью: нет мужской строки у направления → цена общая, пол не выяснять.
    expect(p).toMatch(/если мужского прайса у направления в блоке нет — цена общая для всех/i);
    // Запрет называть цену единицы Ботулакса сохранён и в catalog-ветке.
    expect(p).toMatch(/Цену обобщённой услуги «Ботулинотерапия Ботулакс 1 ед» пациенту НЕ называй/i);
  });

  test('legacy-режим: правило БОТУЛИНОТЕРАПИИ не тронуто (считает по услугам направления)', () => {
    const p = buildSystemPrompt({});
    expect(p).toMatch(/и в диапазон направления НЕ включай — это стоимость одной единицы препарата/i);
    expect(p).not.toMatch(/ДИАПАЗОНЫ ЦЕН/);
  });
});
