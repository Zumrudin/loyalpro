...
describe('renderOffers', () => {
  ...
  // Живой E2E 30.07: «Комбинированная чистка лица на космоцевтике Сhristina,GIGI (Израиль)»
  // резалась ровно на «GIGI (» — пациенту такое название называть нельзя, а модель
  // пересказывает витрину своими словами и может процитировать огрызок.
  test('длинное название режется по границе слова, без висящей открывающей скобки', () => {
    const lines = render({
      o1: { chain: [link({ service_title: 'Комбинированная чистка лица на космоцевтике Сhristina,GIGI (Израиль)' })] },
    });
    expect(lines[0]).toBe('o1 — 30.07: 10:30 «Комбинированная чистка лица на космоцевтике Сhristina,GIGI…» (Юлия)');
    expect(lines[0]).not.toMatch(/[(,\-–—]…/);      // огрызка пунктуации перед многоточием нет
  });

  test('название без пробелов всё равно укладывается в лимит', () => {
    const lines = render({ o1: { chain: [link({ service_title: 'А'.repeat(120) })] } });
    expect(lines[0]).toBe(`o1 — 30.07: 10:30 «${'А'.repeat(60)}…» (Юлия)`);
  });
