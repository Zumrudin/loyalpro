...
describe('checkFalseUnavailability', () => {
  ...
  // Инцидент 2026-09-22 (79265824264): цепочка из трёх услуг, «на 18:00 все три
  // процедуры подряд уже не помещаются в расписание» при 18:00 в starts стыковки.
  // Формулировка отказа для ЦЕПОЧКИ — «не помещается/не влезает», прежняя
  // регулярка знала только «занято/недоступно/нет свободного».
  test('формулировки отказа для цепочки: «не помещаются», «не влезают», «не поместится»', () => {
    const free18 = new Set(['18:00']);
    for (const t of [
      'Ирина, к сожалению, на 18:00 все три процедуры подряд уже не помещаются в расписание.',
      'На 18:00 три процедуры не влезают.',
      'В 18:00 цепочка не поместится.',
    ]) {
      expect(checkFalseUnavailability(t, { freeTimes: free18 })).toEqual([{ type: 'false_unavailability', value: '18:00' }]);
    }
  });
